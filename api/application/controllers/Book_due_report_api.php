...
/**
 * Book Due Report API Controller
 * 
 * This controller handles API requests for book due reports
 * showing overdue books with filtering by date range and member type.
 * 
 * @package    CodeIgniter
 * @subpackage Controllers
 * @category   API
 */
class Book_due_report_api extends CI_Controller
{
     ...
     * }
     * 
     * Empty request body {} returns all overdue books for current year
     */
    public function filter()
    {
        try {
            ...
            // Get book due data
            $result = $this->bookissue_model->getBookDueReport($start_date, $end_date, $member_type);
            ...
            log_message('error', 'Book Due Report API Error: ' . $e->getMessage());
            $this->output
    ...
    public function list()
    {
        try {
            ...
            $response = [
                ...
                'note' => 'Use the filter endpoint with search_type, member_type, or custom date range (from_date, to_date) to get book due report',
                'timestamp' => date('Y-m-d H:i:s')
            ];
