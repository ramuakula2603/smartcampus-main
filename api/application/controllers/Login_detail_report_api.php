...
/**
 * Login Detail Report API Controller
 * 
 * Provides API endpoints for retrieving student login credential information
 ...
 * @version    1.0.0
 */
class Login_detail_report_api extends CI_Controller
{
    /**
     * Constructor
     *
    ...
    /**
     * Filter Login Detail Report
     * 
     * Retrieves student login credentials based on optional filter parameters.
     ...
     * 
     * @method POST
     * @route  /api/login-detail-report/filter
     * 
    ...
    public function filter()
    {
        try {
            ...
            // Get login detail report data from model
            $data = $this->student_model->getLoginDetailReportByFilters($class_id, $section_id, $session_id);
            ...
            log_message('error', 'Login Detail Report API Filter Error: ' . $e->getMessage());
     ...
     * 
     * Retrieves all student login credentials for the current session.
     * No filter parameters required - returns all active students.
     * 
     * @method POST
     * @route  /api/login-detail-report/list
     * 
    ...
    public function list()
    {
        try {
            ...
            // Get all login details for current session (no filters)
            $data = $this->student_model->getLoginDetailReportByFilters(null, null, $session_id);
            ...
            log_message('error', 'Login Detail Report API List Error: ' . $e->getMessage());
