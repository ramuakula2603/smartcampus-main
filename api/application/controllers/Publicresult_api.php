...
class Publicresult_api extends CI_Controller
{
    ...
    public function search()
    {
        ...
        if ($method != 'POST') {
            ...
            if ($this->validate_headers()) {
                    ...
                    $hallticket_no = isset($params['hallticket_no']) ? $params['hallticket_no'] : '';
                    $academic_id = isset($params['academic_id']) ? $params['academic_id'] : '';
                    $exam_id = isset($params['exam_id']) ? $params['exam_id'] : '';

                    if (empty($hallticket_no) || empty($academic_id) || empty($exam_id)) {
                        json_output(400, array('status' => 400, 'message' => 'Hall Ticket Number, Academic Year, and Exam are required.'));
                        return;
                    }

                    // Get student ID
                    $student_id = $this->publicexamtype_model->getstudentid($hallticket_no);
                    ...
                    // Get result name
                    $result_name = $this->publicexamtype_model->getresultname($exam_id);

                    // Get student data
                    $student_data = $this->publicexamtype_model->gtstudentdata($student_id);

                    // Get result status
                    $result_status = $this->publicexamtype_model->getresultstatus($student_id, $exam_id, $academic_id);

                    // Get student results
                    $result_data = $this->publicexamtype_model->getstudentresults($hallticket_no, $exam_id, $academic_id);
    ...
    public function get_sessions()
    {
        ...
        if ($method != 'GET') {
            ...
            if ($this->validate_headers()) {
                $sessions = $this->publicexamtype_model->sessions();
    ...
    /**
     * Get external result types (public exam types)
     * 
     * @return void Outputs JSON response with list of external result types
     */
    public function get_external_result_types()
    {
        $method = $this->input->server('REQUEST_METHOD');
        if ($method != 'GET') {
            json_output(400, array('status' => 400, 'message' => 'Bad request.'));
    ...
    public function search_students()
    {
        ...
        if ($method != 'POST') {
            ...
            if ($this->validate_headers()) {
                ...
                $external_result_type_id = isset($params['external_result_type_id']) ? $params['external_result_type_id'] : '';
                ...
                if (empty($class_id) || empty($section_id) || empty($external_result_type_id)) {
                    json_output(400, array('status' => 400, 'message' => 'Class, Section, and External Result Type are required.'));
                    return;
                }

                // Build query to get students
                $this->db->select('students.id, students.admission_no, students.firstname, students.middlename, students.lastname, students.roll_no, students.mobileno, students.email, classes.class, sections.section, publicresultaddingstatus.assign_status');
                $this->db->from('students');
                $this->db->join('student_session', 'student_session.student_id = students.id');
                $this->db->join('classes', 'classes.id = student_session.class_id');
                $this->db->join('sections', 'sections.id = student_session.section_id');
                $this->db->join('publicresultaddingstatus', 'publicresultaddingstatus.stid = students.id AND publicresultaddingstatus.resultype_id = ' . $this->db->escape($external_result_type_id) . ($session_id ? ' AND publicresultaddingstatus.session_id = ' . $this->db->escape($session_id) : ''), 'left');
                ...
                if ($result_adding_status === 'not_added') {
                    $this->db->where('(publicresultaddingstatus.assign_status IS NULL OR publicresultaddingstatus.assign_status = 0)');
                } elseif ($result_adding_status === 'added') {
                    $this->db->where('publicresultaddingstatus.assign_status', 1);
                }
