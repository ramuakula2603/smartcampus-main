...
/**
 * Student Webservice Controller
 * 
 * This controller provides RESTful API endpoints for student management operations.
 * It mirrors the functionality of the web-based student admission/creation form.
 * 
 ...
 * @version    1.0.0
 */
class Student_webservice extends CI_Controller
{
    /**
     * Constructor
     * 
    ...
    public function create()
    {
        try {
            ...
            $result = $this->process_student_creation($json_input);
     ...
     */
    private function process_student_creation($input_data)
    {
        ...
        return $this->create_student_record($input_data);
    }
     ...
     */
    private function create_student_record($input_data)
    {
        // Start transaction
        $this->db->trans_start();
        ...
        try {
            ...
            if ($this->db->trans_status() === FALSE) {
                return array(
                    'status' => 0,
                    'message' => 'Transaction failed while creating student',
                    'timestamp' => date('Y-m-d H:i:s')
                );
            }
