...
class Addpublicresult_model extends MY_Model
{
    ...
    public function hallticketnostatusgetDatatableByClassSection($class_id, $section_id = null, $status, $resulttype)
    {
        $this->db
            ...
            if ($status == 0) {
                $this->db->where('students.id NOT IN (SELECT stid FROM publicresultaddingstatus WHERE resultype_id = ' . $resulttype . ' AND session_id = ' . $this->current_session . ')');
            }
    ...
    public function subjectsgroup($resulttype){
        ...
        $this->db->select('publicresultsubject_group_subjects.subject_id, publicresultsubject_group_subjects.minmarks, publicresultsubject_group_subjects.maxmarks, resultsubjects.examtype, resultsubjects.subject_code')
    ...
    public function subjectsgroupp($resulttype,$session){
        ...
        $this->db->select('publicresultsubject_group_subjects.subject_id as subid, resultsubjects.subject_code, resultsubjects.examtype as subject_name, publicresultsubject_group_subjects.*')
    ...
    public function searchstudentresultassign($class=null,$section_id=null,$catagoryid=null,$gender=null,$id,$sessionid){
        ...
        $this->db->join('publicresultaddingstatus','students.id = publicresultaddingstatus.stid AND publicresultaddingstatus.resultype_id = ' . $this->db->escape($id), 'left');
    ...
    public function updatingresult($stid,$resid,$data,$sessionid){
        ...
        $query = $this->db->get('publicresultaddingstatus');
        ...
        if ($query->num_rows() > 0) {
            ...
            $this->db->insert('publicresultaddingstatus', $insert_data);
        }
