# Staff Attendance Years API Documentation

## Overview

The **Staff Attendance Years API** provides endpoints to retrieve the list of available years with staff attendance records in the school management system. This API is designed to populate year dropdowns in the staff attendance report interface.
...
Retrieves a simple list of distinct years that have staff attendance records.

**Endpoint:** `POST /api/staff-attendance-years/list`
...
Retrieves years with additional statistics including total records, total staff, earliest date, and latest date.

**Endpoint:** `POST /api/staff-attendance-years/details`
...
| `data[].year` | String | Year value (YYYY format) |
| `data[].total_records` | Integer | Total staff attendance records in this year |
| `data[].total_staff` | Integer | Total unique staff with attendance in this year |
...
```bash
curl -X POST http://localhost/amt/api/staff-attendance-years/list \
...
```bash
curl -X POST http://localhost/amt/api/staff-attendance-years/details \
  -H "Content-Type: application/json" \
  -H "Client-Service: smartschool" \
  -H "Auth-Key: schoolAdmin@" \
  -d '{}'
...
```javascript
async function getStaffAttendanceYears() {
  try {
    const response = await fetch('http://localhost/amt/api/staff-attendance-years/list', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
...
```php
<?php
$url = 'http://localhost/amt/api/staff-attendance-years/list';
...
When loading the staff attendance report page, call the `/list` endpoint to populate the year dropdown:
...
async function showYearStatistics() {
    const response = await fetch('http://localhost/amt/api/staff-attendance-years/details', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'Client-Service': 'smartschool',
    ...
    if (data.status === 1) {
        data.data.forEach(year => {
            ...
            console.log(`  Staff: ${year.total_staff}`);
...
Create `test_staff_attendance_years_api.php`:

```php
<?php
/**
 * Test Script for Staff Attendance Years API
 */
...
echo "=== STAFF ATTENDANCE YEARS API TESTS ===\n\n";

// Test 1: List Years
echo "Test 1: List Available Years\n";
$result = call_api("{$base_url}/staff-attendance-years/list", $headers);
...
$result = call_api("{$base_url}/staff-attendance-years/details", $headers);
if ($result['http_code'] == 200 && $result['response']['status'] == 1) {
    echo "✓ SUCCESS\n";
    echo "  Total Years: {$result['response']['total_years']}\n";
    foreach ($result['response']['data'] as $year) {
...
Run the test:
```bash
C:\xampp\php\php.exe test_staff_attendance_years_api.php
```
...
Load available years when the staff attendance report page loads to populate the year filter dropdown.

### 2. Date Range Validation
Use the earliest and latest dates to validate user-selected date ranges.

### 3. Data Availability Check
Check if staff attendance data exists for a specific year before allowing report generation.

### 4. Statistics Dashboard
Display year-wise staff attendance record counts and staff participation in a dashboard.
...
- **Class Attendance Years API** - `/api/class-attendance-years/list`
...
### Controller
- **File:** `api/application/controllers/Staff_attendance_years_api.php`
- **Class:** `Staff_attendance_years_api`
- **Extends:** `CI_Controller`

### Model
- **File:** `api/application/models/Staffattendancemodel.php`
- **Methods:**
  - `getStaffAttendanceYears()` - Get simple year list
  - `getStaffAttendanceYearsWithDetails()` - Get years with statistics
...
### Database Tables
- `staff_attendance` - Stores all staff attendance records
...
**Causes:**
- No staff attendance records in database
- All attendance dates are NULL

**Solution:**
- Verify attendance data exists in `staff_attendance` table
