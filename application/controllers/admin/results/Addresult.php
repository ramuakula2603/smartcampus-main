...
class Addresult extends Admin_Controller
{
        ...
        if ($this->input->server('REQUEST_METHOD') == "GET") {
            ...
            if (isset($search)) {
                ...
                if ($this->form_validation->run() == false) {
                    ...
                    // Debug: Log the subjects data to verify minmarks and maxmarks are retrieved
                    log_message('debug', 'search method - Retrieved subjects: ' . print_r($data['subjects'], true));
    ...
    public function addmore_point()
    {
        ...
        // Debug: Log the received subjectsData to check if minmarks and maxmarks are present
        log_message('debug', 'addmore_point - Received subjectsData: ' . print_r($data['subjectsData'], true));
        ...
        if (!empty($_POST['actualmarks'])) {
            foreach ($_POST['actualmarks'] as $maks => $maksval) {
                if ($maksval == '') {
                  ...
                  // Allow numeric values or "AB" for absent
                  $maksval_upper = strtoupper(trim($maksval));
                  $expr = '/^[0-9]*(\.[0-9]{0,2})?$/';

                  if ($maksval_upper !== 'AB' && !preg_match($expr, $maksval)) {
                   $validate            = 0;
                   $msg['actualmarks'] = "<p>Please enter valid marks or 'AB' for absent</p>";
                   break;
                }
        ...
        if ($validate == 0) {
            ...
            if (!empty($_POST['pickup_point_id'])) {
                ...
                foreach ($_POST['pickup_point_id'] as $pickup_pointkey => $pickup_pointvalue) {
                    ...
                    // Normalize actualmarks - convert "ab" or "Ab" to "AB"
                    $actualmarks = $this->input->post('actualmarks')[$pickup_pointkey];
                    if (strtoupper(trim($actualmarks)) === 'AB') {
                        $actualmarks = 'AB';
                    }
