...
class Examtype_model extends MY_Model
{
    ...
    public function getstudentid($admino){
        // FIXED: Look up student by admission_no in students table (not student_admi table)
        // CSV contains admission numbers that match students.admission_no
        $query = $this->db->select('id')
                          ->from('students')
                          ->where('admission_no', $admino)
                          ->get();
        ...
        if ($query->num_rows() > 0) {
            // Admission number found, return student ID
            return $query->row()->id;
        } else {
            // Admission number not found, return false
            return false;
        }
    }
