...
class Internalresult_model extends CI_Model
{
    ...
    /**
     * Get internal results report based on filters
     ...
     */
    public function getInternalResultsReport($filters = array())
    {
        try {
            ...
            $this->db->select('
                ...
                sessions.session,
                examtype.id as exam_type_id,
                examtype.examtype as exam_name,
                ...
                internalresulttable.id as result_id
            ');
            ...
            if (!empty($filters['session_id']) && is_array($filters['session_id'])) {
                $this->db->where_in('internalresulttable.session_id', $filters['session_id']);
            }

            if (!empty($filters['exam_type_id']) && is_array($filters['exam_type_id'])) {
                $this->db->where_in('internalresulttable.resulgroup_id', $filters['exam_type_id']);
            }
            ...
            log_message('error', 'Error fetching internal results report: ' . $e->getMessage());
            return array();
        }
    }
    ...
    /**
     * Get distinct sessions that have internal results
