...
$(document).on('input','.chat_input',function(){
    ...
    // Handler for Generate PDF buttons (uses server embed=true fallback)
    $(document).on('click', '.generate-pdf', function (e) {
        e.preventDefault();
        var $btn = $(this);
        $btn.prop('disabled', true);

        // Collect selected messages; fall back to all messages if none selected
        var messages = [];
        $('.messages ul li.selected p').each(function () {
        ...
        $.ajax({
            type: 'POST',
            url: baseurl + 'admin/Generatepaper/generate_pdf',
            data: {messages: messages, embed: 'true'},
            dataType: 'JSON',
            success: function (resp) {
                ...
                if (resp.format && resp.format === 'html' && resp.html) {
                    try {
                        var html = atob(resp.html);
                        ...
                        console.error('Failed to decode HTML fallback', e);
                        alert('Failed to open HTML fallback for printing.');
                    }
                } else if (resp.status && resp.status === 'error') {
                    alert('Error: ' + (resp.message || 'Unknown error'));
                } else {
                    // If server returned a base64 PDF (older behavior), open it
                    if (resp.pdf_data) {
                        var byteChars = atob(resp.pdf_data);
            ...
            error: function (xhr, status, err) {
                alert('Failed to contact server: ' + status);
            },
            complete: function () {
